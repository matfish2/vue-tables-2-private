    ...
    var client = merge.recursive(true, Table(), {
        ...
        methods: {
            applyInfiniteScroll() {
                ...
                this.$nextTick(() => {
                    let nextObserver = new IntersectionObserver(entries => {
                        entries.forEach((e) => {
                            if (e.isIntersecting && new Date().getTime()-lastTime>10) {
                                this.setPage(this.page + 1);
                                lastTime = new Date();
                                console.log("intersection" + this.page)
                                this.$nextTick(()=>{
                                nextObserver.unobserve(this.currentTarget);
                                this.applyInfiniteScroll();
                                })
                            } else {
                        ...
                        root: this.$el.querySelector('.table-responsive'),
                        // rootMargin: '-30px',
                        // threshold: 1.0
                    });

                    let prevObserver = new IntersectionObserver(entries => {
                        entries.forEach((e) => {
                            if (e.isIntersecting) {
                        ...
                        nextObserver.observe(this.currentTarget)
                            }
